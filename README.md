...
Investing in the Money desk (`/debt.html`, **Investing**) charts the player against "if you had held" and a 90/10 autopilot on the same seeded market, pauses time for a decision when a bear market starts, and at the recovery shows what the choice cost, with a lesson from the Gemini coach written from the run's own Tiger Data ([spec](docs/superpowers/specs/2026-09-12-investing-twins-design.md)).
