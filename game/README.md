...
- **Phone:** the hub for the game's apps, docked on the left edge (tuck it away with its side tab). Stocks shows a market watchlist and opens the Credit Desk over the city (city time pauses while it is open); News, Mail, and Bank are coming.
- **Money:** every game day the player is paid on the 1st and 15th, pays rent and living costs for the current state, and pays their debts; the home tier follows net worth, and bankruptcy stops a skip.
...
- `src/sim/life/`: the player's money life (`PlayerLife`): paychecks, state-scaled rent and living costs, the accounts ledger, and the debt engine, run from `Clock.onDay` in `src/main.ts`; `rates.ts` reads the market snapshot.
- `src/ui/phone.ts`: the phone hub and its app registry (add new apps to `APPS`); `src/ui/hud.ts`, `usmap.ts`, and `npccard.ts` are the rest of the DOM HUD.
...
| `npm test` | Node's built-in test runner over `tests/` (debt engine, money life, money, and Card Shop); no test dependencies |
