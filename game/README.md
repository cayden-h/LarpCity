...
- `debt.html` + `src/debt-demo/main.ts`: the Money desk in Robinhood's look: Home (net worth), Cash (accounts, transfers, and a bank-style statement), Investing, Debt, Credit, and Cards, running its own `PlayerLife` with the starter portfolio (`STARTER_PORTFOLIO`), so net worth moves with the market from day one and charts show the real market before day 0.
