Status: Implemented and merged into `GameEngine` on 2026-09-12 via branch `npc-bank-interaction` (all 21 tasks, task-by-task reviewed, plus a final whole-branch review). Moved here from the now-retired `GameEnginePlan.md` to match this directory's convention (see `docs/superpowers/plans/2026-09-12-nessie-fallback.md`) once that file's other sections were split out to `docs/superpowers/specs/`.
