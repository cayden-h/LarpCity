...
Re-read it, [docs/meetings/2026-09-13-revamp.md](docs/meetings/2026-09-13-revamp.md) (the latest), and [docs/meetings/2026-09-11-game-design.md](docs/meetings/2026-09-11-game-design.md) before assuming a mechanic is still current; a later meeting wins over an earlier one, and both win over `research/`.

The app is two parts: the browser game in `game/` and the API server in `server/`.
Everything else (`research/`, `docs/`) is design docs, data builders, and assets that feed them.
...
npm run debt:charts       # regenerate docs/diagrams/debt/05-payoff-strategies.svg from the live engine
```
...
- `sim/life/` (`PlayerLife`): the player's daily life: paychecks (1st and 15th), state-scaled rent and living costs, brokerage holdings and recurring buys, standing orders (401(k) match, emergency fund, crash rule), and the starter portfolio (`STARTER_PORTFOLIO`).
  ...
  `intake.ts` (`lifeFromIntake`, `defaultAnswers`, `starterFor`) builds every new life from the same default start, with its job, salary, and debt drawn from the seed; the player never states money, only the choices in Sammy's setup (`ui/intake.ts`).
  `events.ts`'s `EVENT_RATES` were halved on 2026-09-13 (about 13 events in a 40-year life).
...
- `sim/rewind/` (`LifeTimeline`): a checkpoint of a life for every game day, restored into the same `PlayerLife`, so the Calendar goes back to any past day exactly, but only once `gate.ts` (`ReviewGate`) opens at the end-of-game review; `rewindTo` in `src/main.ts` rewinds the clock, NPCs, bank mirror, and recorder (which forks the run on the server) with it.
...
- `sim/save/`: the whole game as one saved document (`GameSave`, `toSave`/`fromSave` on each stateful sim class), `codec.ts` (`restoreGame`), `boot.ts` (resume, from-profile, intake, or offline), and `manager.ts` (`SaveManager`, autosaving on decisions, new months, and skips, and treating a 409 as another tab now owning the life), and `slot.ts` (the player's three save slots and the judges' demo lives, built by `npm run demo:slots` into `public/demo/`).
...
The HUD (`hud.ts`) is just the "Your home" card; the phone (`phone.ts`, pulled up from the bottom-right) is the hub: Stocks (sponsor stocks and markets, opens the Money desk), Goals (the fast-forward, `skip-setup.ts`), Map (San Francisco only: the game has one place, and any other state hash falls back to `#CA`), Calendar (`calendar.ts`: the month and year views, going back to a past day once the end-of-game review opens it, skipping to the next decision day, the speed, and "Start a new life" and "Save slots" (`slots.ts`) from the year view), and the live Mail, News, and Bank apps (`phone-apps.ts`), reading the life's inbox, the Ledger, and the Nessie statement.
Add new phone apps to `APPS`.
The onboarding is `title.ts` (the title screen and Sammy's Learn walkthrough from `src/narration/learn.ts`), then `slots.ts` full screen (pick a save slot), then `intake.ts` for a new life (Sammy presents the generated life; the player picks a name and avatar, a health plan, a starter card, and the four goals with their target ages); `?slot=` in the URL boots straight into a slot.
`narrator.ts` with `src/narration/lines.ts` is Sammy, the owl narrator, and his pre-voiced lines; `tour.ts` with `src/narration/tour.ts` and `tours.ts` are his guided tours (stocks and taxes).
...
- `docs/meetings/`: the decision log, one file per meeting.
- `docs/diagrams/debt/`: pitch-deck SVGs, some regenerated by `npm run debt:charts`.
- `docs/screenshots/` (our game) and `docs/reference/` (the LEGO reference game).
- `docs/notion/`, `docs/history/`: the original Notion notes, pre-edit backups of the team's Notion page, and resolved integration records; the live source of truth is Notion, and these are historical snapshots.
- Keep the repo root to the app (`game/`, `server/`), `research/`, `docs/`, and the top-level READMEs; new docs go under `docs/`.
